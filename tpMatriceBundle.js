...
/***/ function(module, exports, __webpack_require__) {
	...
	var config = [], configOfTouchId = {}, transfo = __webpack_require__(6), zIndex = 0;
	...
	var automataRotoZoom = {
		init : function(conf) {
			...
			conf.node.style.transform = conf.node.style.transform
					|| "translate(0,0)";
			config.push(conf);
		...
		nothing : function(conf, event, touch) {
			...
			conf.touchesId[touch.identifier] = {
				point : transfo.getPoint(touch.pageX, touch.pageY).matrixTransform(
						conf.originalMatrixInv),
		...
		drag : function(conf, event, touch) {
			...
			case "move":
				...
				transfo.dragNode(conf.node, conf.originalMatrix,
						conf.touchesId[touch.identifier].point, conf.currentMatrix,
		...
		rotozoom : function(conf, event, touch) {
			...
			case "move":
				...
				conf.touchesId[touch.identifier].currentPoint.x = touch.pageX;
				conf.touchesId[touch.identifier].currentPoint.y = touch.pageY;
				var idPtr1 = conf.touches[0]
				  , idPtr2 = conf.touches[1];
				transfo.rotoZoomNode(conf.node, conf.originalMatrix,conf.currentMatrix,
						conf.touchesId[idPtr1].point, conf.touchesId[idPtr1].currentPoint,
						conf.touchesId[idPtr2].point, conf.touchesId[idPtr2].currentPoint
						);
				break;
	...
	function startMouse(event) {
		...
		var node = event.currentTarget, conf = config[node.dataset.confId];
	...
	function startTouch(event) {
		event.preventDefault();
		event.stopPropagation();
		var node = event.currentTarget, conf = config[node.dataset.confId], L = event.changedTouches, i, touch;
